		...
		const vector<Attribute> &recordDescriptor, const void *data, const RID &rid)
{
	...
	rid0.pageNum=*(int *)((char *)buffer+*(int *)((char *)buffer-4*slotNum+PAGE_SIZE-8));
	rid0.slotNum=*(int *)((char *)buffer+*(int *)((char *)buffer-4*slotNum+PAGE_SIZE-8)+4);

	int ptr0 = *((int *) ptr - 1 - 2*slotNum);
	int len0 = *((int *) ptr - 1 - 2*slotNum + 1);

	success = success | insertRecord(fileHandle, recordDescriptor, data, tombstone);
	success = success |fileHandle.readPage(pageNum,buffer);
	*(int *)((char *)buffer+*(int *)((char *)buffer-4*slotNum+PAGE_SIZE-8))=tombstone.pageNum;
	*(int *)((char *)buffer+*(int *)((char *)buffer-4*slotNum+PAGE_SIZE-8)+4)=tombstone.slotNum;

	if(rid0.pageNum != -1)
				success = success | deleteRecord(fileHandle,recordDescriptor,rid0);

	success=success|fileHandle.writePage(pageNum,buffer);
	free(buffer);
	return success;
}
		...
		const vector<Attribute> &recordDescriptor, const unsigned pageNumber)
{
	int success=0;
	void *buffer=malloc(PAGE_SIZE);
	success=success|fileHandle.readPage(pageNumber,buffer);

	int freepointer=0;
	void *ptr = buffer + PAGE_SIZE -8;
	int no_of_slots = *((int *) ptr);
	for(int i=1;i<=no_of_slots;i++)
	{
		int initial_ptr=*(int *)(ptr-4*i);
		int rec_length=*(int *)((char *)buffer+initial_ptr);
		if(initial_ptr!=-1)
		{
			if(rec_length!=-1)
			{
				memcpy((char *)buffer+freepointer,(char *)buffer+initial_ptr,8);
				initial_ptr=freepointer;
				freepointer+=8;
			}
			else
			{
				int final=record_length(buffer,initial_ptr+8,recordDescriptor,recordDescriptor.size());
				memcpy((char *)buffer+freepointer,(char *)buffer+initial_ptr,final+8);
				initial_ptr=freepointer;
				freepointer+=final+8;
			}
		}
	}
	*(int *)((char *)buffer+PAGE_SIZE-4)=freepointer;
	success=success|fileHandle.writePage(pageNumber,buffer);
	free(buffer);
	return success;
	//return -1;
}
